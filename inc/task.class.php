...
JAVASCRIPT;
               ...
               // Task user or everyone if settings allow display to all users
               if ($buttons || $config->displayInfoForEveryone()) {
   ...
   static function postShowItem($params) {
      ...
      switch ($params['item']->getType()) {
         case 'TicketTask':
            ...
            // Verify if timer should be shown in the closed task box
            if (self::checkTech($task_id) || $config->displayInfoForEveryone()) {
               $time = self::totalEndTime($task_id);
...
$(document).ready(function() {
   if ($("[id^='actualtime_faclock_{$task_id}_']").length == 0) {
      $("span.state[onclick='change_task_state({$task_id}, this)']")
...
JAVASCRIPT;
            ...
            if ($params['item']->fields['state']==1 && $task_id) {
               $query=[
                  'FROM'=>self::getTable(),
                  'WHERE'=>[
                     'tasks_id'     => $task_id,
